...
namespace Calcpad.Core
{
    internal class HtmlWriter : OutputWriter
    {
        ...
        internal override string FormatSuperscript(string s) => $"<sup>{s}</sup>";
        ...
        internal override string FormatMatrix(Matrix matrix)
        {
            ...
            // Para matrices no-HP: detectar si todos los elementos tienen la misma unidad
            // Si es así, mostrar números puros dentro del bracket y unidad afuera
            Unit commonUnit = null;
            var hasCommonUnit = false;
            if (hp_m is null && matrix.RowCount > 0)
            {
                commonUnit = matrix[0, 0].Units;
                if (commonUnit is not null)
                {
                    hasCommonUnit = true;
                    for (int i = 0; i < matrix.RowCount && hasCommonUnit; i++)
                        for (int j = 0; j < nc && hasCommonUnit; j++)
                        {
                            var cellUnit = matrix[i, j].Units;
                            if (cellUnit is null || cellUnit.Text != commonUnit.Text)
                                hasCommonUnit = false;
                        }
                }
            }
            ...
            for (int i = 0, nr = matrix.RowCount; i < nr; ++i)
            {
                ...
                for (int j = 0; j < nc; ++j)
                {
                    ...
                    if (hp_m is not null)
                    {
                        // HpMatrix: número puro (unidad va afuera)
                        var d = hp_m.GetValue(i, j);
                        s = FormatReal(d, units?.FormatString, zeroSmallElements && Math.Abs(d) < zeroThreshold);
                    }
                    else if (hasCommonUnit)
                    {
                        // Matriz con unidad común: número puro (unidad va afuera)
                        var d = matrix[i, j].D;
                        s = FormatReal(d, commonUnit?.FormatString, zeroSmallElements && Math.Abs(d) < zeroThreshold);
                    }
                    else
                    {
                        // Matriz con unidades mixtas o sin unidades: cada celda con su unidad
            ...
            sb.AppendLine("</span>");
            // Mostrar unidad una sola vez afuera del bracket
            if (units is not null)
                sb.Append(HairSpace).Append(units.Html);
            else if (hasCommonUnit)
                sb.Append(HairSpace).Append(commonUnit.Html);
        ...
        internal override string FormatVector(Vector vector)
        {
            ...
            // Para vectores no-HP: detectar si todos los elementos tienen la misma unidad
            Unit commonUnit = null;
            var hasCommonUnit = false;
            if (hp_v is null && len > 0)
            {
                commonUnit = vector[0].Units;
                if (commonUnit is not null)
                {
                    hasCommonUnit = true;
                    for (int i = 1; i < len && hasCommonUnit; i++)
                    {
                        var cellUnit = vector[i].Units;
                        if (cellUnit is null || cellUnit.Text != commonUnit.Text)
                            hasCommonUnit = false;
                    }
                }
            ...
            // Vectors with more than 4 elements → render as row (horizontal)
            // Vectors with 4 or fewer elements → render as column (vertical)
            if (len > 4)
                FormatVectorAsRow();
            else
            ...
            // Render as column: each element on its own row
            void FormatVectorAsColumn()
            {
                sb.AppendLine("<span class=\"matrix\">");
                for (int i = 0; i < len; ++i)
                {
            ...
            void AppendElement(int index)
            {
                if (hp_v is not null)
                {
                    // HpVector: número puro (unidad va afuera)
                    var d = hp_v.GetValue(index);
                    sb.Append(FormatReal(d, units?.FormatString, zeroSmallElements && Math.Abs(d) < zeroThreshold));
                }
                else if (hasCommonUnit)
                {
                    // Vector con unidad común: número puro (unidad va afuera)
                    var d = vector[index].D;
                    sb.Append(FormatReal(d, commonUnit?.FormatString, zeroSmallElements && Math.Abs(d) < zeroThreshold));
                }
                else
                {
                    // Vector con unidades mixtas o sin unidades
