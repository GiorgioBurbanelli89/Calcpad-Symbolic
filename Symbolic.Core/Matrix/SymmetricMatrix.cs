...
namespace Calcpad.Core
{
    internal class SymmetricMatrix : Matrix
    {
        ...
        private const int EigenThreshold = 100;
        ...
        {
            // Eigen C++ dense solver path
            int n = _rowCount;
            // Extraer matriz densa (row-major, full matrix desde upper triangular)
            var A = new double[n * n];
            for (int i = 0; i < n; i++)
                for (int j = i; j < n; j++)
                {
                    double val = this[i, j].D;
                    A[i * n + j] = val;
                    A[j * n + i] = val; // simetría
                }
            ...
            var x = new double[n];
            int result = EigenInterop.dense_solve(n, A, rhs, x);
            if (result < 0)
                throw Exceptions.MatrixSingular();

            var rv = new RealValue[n];
            for (int i = 0; i < n; i++)
                rv[i] = new RealValue(x[i]);
