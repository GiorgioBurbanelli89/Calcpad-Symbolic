    ...
    internal class Unit : IEquatable<Unit>
    {
        ...
        internal static string GetProductOrDivisionOperator(OutputWriter.OutputFormat format, bool isProduct)
        {
            if (isProduct)
            {
                ...
                return TextWriter.UnitProduct.ToString();
            }
            if (format == OutputWriter.OutputFormat.Xml)
            ...
            return TextWriter.UnitDivision.ToString();
        }
        ...
        internal bool IsConsistent(Unit other)
        {
        ...
        /// <summary>
        /// Operador &amp;: convierte valor de this-unit al sistema de targetUnit por dimensión.
        /// Para cada dimensión i del source: convierte source→SI, luego SI→target.
        /// Si la dimensión no existe en target, solo convierte source→SI (factor base=1).
        /// Resultado: factor tal que value_new = value_old * factor, con unidad=targetUnit.
        /// </summary>
        internal double ForceConvertTo(Unit targetUnit)
        {
            if (IsDimensionless)
                return 1d;
            ...
            var factor = 1d;
            var n = Length;
            var tn = targetUnit.Length;
            var tf = targetUnit._factors;
            var tp = targetUnit._powers;
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                // source_factors[i]^source_powers[i] convierte source→SI para dimensión i
                var toSI = MyPow(_factors[i], _powers[i]);
                factor *= toSI;

                // Si target tiene esta dimensión, convertir SI→target
                if (i < tn && tp[i] != 0f)
                {
                    // target_factors[i]^target_powers[i] convierte target→SI
                    // Inverso = SI→target
                    var fromSI = MyPow(tf[i], tp[i]);
                    factor /= fromSI;
                }
            }
            return factor;
        ...
        internal static Unit Divide(Unit ua, Unit ub, out double d, bool updateText = false)
        {
            ...
            if (ua is null)
            {
                if (ub.IsDimensionless)
                {
                    d = 1d / ub.GetDimensionlessFactor();
                    return null;
                }
                d = 1d;
                return ub.Pow(-1f);
        ...
        /// <summary>
        /// Calcula el factor para convertir 1 unidad a SI base (g,m,s,A,...).
        /// Ejemplo: 1 kN = GetSIFactor() g·m/s² (en base g,m,s)
        /// </summary>
        internal double GetSIFactor()
        {
            if (IsDimensionless) return GetDimensionlessFactor();
            var factor = 1d;
            for (int i = 0, n = Length; i < n; ++i)
            {
                if (_powers[i] != 0f)
        ...
        internal bool IsEmpty() => _powers.Length == 0 || Array.TrueForAll(_powers, p => p == 0f);
