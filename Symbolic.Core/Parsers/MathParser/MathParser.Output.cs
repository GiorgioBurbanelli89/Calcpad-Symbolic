    ...
    public partial class MathParser
    {
        private sealed class Output
        {
            ...
            internal string Render(OutputWriter.OutputFormat format)
            {
                ...
                else
                {
                    ...
                    if (_parser._isCalculated)
                    {
                        ...
                        // e.g. va = [-1; 0; 1] should NOT show [-1 0 1] = [-1 0 1]
                        // Check RPN tokens AFTER assignment: only constants/units/operators allowed
                        var isConstantLiteral = false;
                        if (!_hasVariables && _assignmentPosition > 0 &&
                            (_parser._result is Vector || _parser._result is Matrix))
                        {
                            isConstantLiteral = true;
                            // Find assignment token index, then check everything after it
                            int assignIdx = -1;
                            for (int i = 0; i < len; i++)
                            {
                                if (rpn[i].Type == TokenTypes.Operator &&
                                    (rpn[i].Content == "=" || rpn[i].Content == "←"))
                                { assignIdx = i; break; }
                            }
                            // Check tokens between start and assignment (skip the LHS variable)
                            for (int i = 0; i < (assignIdx > 0 ? assignIdx : len); i++)
                            {
                                var tt = rpn[i].Type;
                                // Skip the first variable (LHS of assignment)
                                if (i == 0 && (tt == TokenTypes.Variable ||
                                    tt == TokenTypes.Vector || tt == TokenTypes.Matrix))
                                    continue;
                                if (tt == TokenTypes.Variable || tt == TokenTypes.Function ||
                                    tt == TokenTypes.Function2 || tt == TokenTypes.Function3 ||
                                    tt == TokenTypes.VectorFunction2 || tt == TokenTypes.VectorFunction3 ||
                                    tt == TokenTypes.MatrixFunction2 || tt == TokenTypes.MatrixFunction3 ||
                                    tt == TokenTypes.MatrixFunction4 || tt == TokenTypes.MatrixFunction5 ||
                                    tt == TokenTypes.MatrixIterativeFunction ||
                                    tt == TokenTypes.CustomFunction)
                                {
                                    isConstantLiteral = false;
                                    break;
                                }
                            }
                        }
                        if (!isConstantLiteral &&
                            (hasOperators && res != subst || string.IsNullOrEmpty(subst)))
                        {
                        ...
                        // Doble igualdad para vectores/matrices con variables:
                        // Si ya se mostró la forma simbólica K = [k₁ 0 | 0 k₂],
                        // agregar = [valor valor | valor valor] con resultado numérico
                        else if (!isConstantLiteral && _hasVariables &&
                            (_parser._result is Vector || _parser._result is Matrix) &&
                            !string.IsNullOrEmpty(res) && res != subst)
                        {
                            _stringBuilder.Append(assignment);
                            _stringBuilder.Append(res);
                        }
            ...
            private string RenderRpn(Token[] rpn, bool substitute, OutputWriter writer, out bool hasOperators)
            {
                ...
                void RenderVariableToken(RenderToken t, int i)
                {
                    ...
                    else if (ival is Matrix matrix)
                    {
                        var s = !_parser._settings.Substitute &&
                            ...
                            string.Empty;
                        // Vector (1 row or 1 col) gets arrow, matrix does not
                ...
                {
                    var st = t.Content;
                    // Mathematical notation for matrix functions
                    if (string.Equals(st, "transp", StringComparison.OrdinalIgnoreCase) ||
                        string.Equals(st, "transpose", StringComparison.OrdinalIgnoreCase))
                    {
                        // transp(B) → Bᵀ (superscript T)
                        t.Content = b.Content + writer.FormatSuperscript("T");
                        t.Level = b.Level;
                    }
                    else if (string.Equals(st, "inverse", StringComparison.OrdinalIgnoreCase))
                    {
                        // inverse(A) → A⁻¹ (superscript -1)
                        t.Content = b.Content + writer.FormatSuperscript("−1");
                        t.Level = b.Level;
                    }
                    else if (string.Equals(st, "det", StringComparison.OrdinalIgnoreCase))
                    {
                        // det(A) → |A| (vertical bars)
                        t.Content = "|" + b.Content + "|";
                        t.Level = b.Level;
                    }
                    else
                ...
                void RenderFunction2Token(RenderToken t, RenderToken b)
                {
                    ...
                    else if (string.Equals(st, "lsolve", StringComparison.OrdinalIgnoreCase) ||
                             string.Equals(st, "clsolve", StringComparison.OrdinalIgnoreCase))
                    {
                        // lsolve(A; b) → A⁻¹·b
                        t.Level = Math.Max(a.Level, b.Level);
                        t.MinOffset = Math.Min(a.MinOffset, b.MinOffset);
                        t.MaxOffset = Math.Max(a.MaxOffset, b.MaxOffset);
                        t.Content = a.Content + writer.FormatSuperscript("−1") +
                            writer.FormatOperator('*') + b.Content;
                    }
                    else
                    {
                        t.Level = Math.Max(a.Level, b.Level);
                ...
                {
                    // Collect each row as a separate string
                    var nRows = t.ParameterCount + 1;
                    var rows = new string[nRows];
                    rows[nRows - 1] = b.Content;
                    t.Level = b.Level;
                    ...
                    for (int j = nRows - 2; j >= 0; --j)
                    {
                        var a = stackBuffer.Pop();
                        rows[j] = a.Content;
                        if (a.Level > t.Level) t.Level = a.Level;
                        t.MinOffset = Math.Min(a.MinOffset, t.MinOffset);
                        t.MaxOffset = Math.Max(a.MaxOffset, t.MaxOffset);
                    }
                    // Build matrix HTML with <span class="matrix"> (same as result format)
                    var sb = new System.Text.StringBuilder();
                    sb.Append("<span class=\"matrix\">");
                    foreach (var row in rows)
                    {
                        sb.Append("<span class=\"tr\"><span class=\"td\"></span>");
                        // Split row by semicolon separator (rendered as "; ")
                        var cells = row.Split(new[] { div }, StringSplitOptions.None);
                ...
                void RenderVectorToken(RenderToken t, RenderToken b)
                {
                    ...
                    var nElems = t.ParameterCount + 1;
                    var elems = new string[nElems];
                    elems[nElems - 1] = b.Content;
                    t.Level = b.Level;
                    t.MinOffset = b.MinOffset;
                    t.MaxOffset = b.MaxOffset;
                    for (int j = nElems - 2; j >= 0; --j)
                    {
                        var a = stackBuffer.Pop();
                        elems[j] = a.Content;
                        if (a.Level > t.Level) t.Level = a.Level;
                    ...
                    // Build row vector HTML with <span class="matrix"> (horizontal)
                    var sb = new System.Text.StringBuilder();
                    sb.Append("<span class=\"matrix\">");
                    sb.Append("<span class=\"tr\"><span class=\"td\"></span>");
                    foreach (var elem in elems)
                    {
                        sb.Append($"<span class=\"td\">{elem.Trim()}</span>");
                    }
                    sb.Append("<span class=\"td\"></span></span>");
                    sb.Append("</span>");
                    t.Content = sb.ToString();
