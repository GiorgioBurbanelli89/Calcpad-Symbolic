...
namespace Calcpad.Core
{
    public partial class MathParser
    {
        ...
        private Unit[] _targetUnitsArray; // | [u1;u2;...]: conversión por elemento
        private Unit _forceUnits;       // Operador & escalar: fuerza unidad
        private Unit[] _forceUnitsArray; // Operador & [u1;u2;...]: vector/matriz de unidades
        private int _forceUnitsRows;    // Filas del array de unidades (0=vector)
        private int _forceUnitsCols;    // Columnas del array de unidades
        ...
        internal IValue ResultValue => _result;
        ...
        // Set variable to any IValue (scalar, vector, or matrix) — used by #function
        internal void SetVariable(string name, IValue value)
        {
            if (value is IScalarValue scalar)
            {
                SetVariable(name, scalar);
                return;
            }
            if (_variables.TryGetValue(name, out var variable))
            ...
            {
                var v = new Variable(RealValue.Zero);
                v.SetValue(value);
                _variables.Add(name, v);
                _input.DefinedVariables.Add(name);
            }
        ...
        internal void RemoveVariable(string name)
        {
            ...
            _input.DefinedVariables.Remove(name);
        }
        ...
        // Ej: [10; 5]kN → [10; 5]*kN
        // Retorna null si no hay cambios (para evitar allocar string innecesariamente)
        private static string InsertImplicitMultiplyAfterBracket(ReadOnlySpan<char> expression)
        {
            for (int i = 0; i < expression.Length - 1; i++)
            {
                if (expression[i] == ']' && Validator.IsLetter(expression[i + 1]))
                {
                    // Encontramos ]letra — necesitamos insertar *
                    // Pero verificar que no sea un índice como v.(1) seguido de algo
                    var sb = new StringBuilder(expression.Length + 4);
                    sb.Append(expression[..(i + 1)]);  // Todo hasta ]
                    sb.Append('*');                      // Insertar *
                    sb.Append(expression[(i + 1)..]);   // Resto desde la letra
                    return sb.ToString();
                }
            }
            ...
            if (_functionDefinitionIndex < 0)
            {
                ...
                else
                {
                    ...
                    if (isAssignment && _rpn[0] is VariableToken vt)
                    {
                        ...
                        // & [array] ya se aplicó en el Evaluator — no re-aplicar
                        if (_forceUnitsArray is not null)
                            Units = null;
                        else if (_forceUnits is not null)
                            Units = Evaluator.ForceApplyUnits(ref val, _forceUnits);
                        else
